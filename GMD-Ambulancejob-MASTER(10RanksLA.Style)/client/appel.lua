...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	exports['t-notify']:Custom({
		...
		duration  =  10500,
		title  =  'EMS',
		message  =  'Es braucht jemand einen Mediziner. Willst du den Notruf annehmen? Drücke ~g~Z~g~ oder lehne diesen Auftrag mit ~r~X~r~ ab!',
		sound  =  true
	})
end)
     ...
     while true do
		...
		if AppelEnAttente then
			...
			elseif IsControlJustPressed(1, 73) and AppelEnAttente then
				...
				exports['t-notify']:Custom({
					...
					duration  =  10500,
					title  =  'EMS',
					message  =  'Du hast diesen Auftrag abgelehnt, vonwegen freund und helfer.',
					sound  =  true
				})
		...
		if IsControlJustPressed(1, 246) and AppelDejaPris == true then
			...
			exports['t-notify']:Custom({
				...
				duration  =  10500,
				title  =  'EMS',
				message  =  'Der Notruf wurde schon von einer anderen Einheit angefahren!',
				sound  =  true
			})
		end
     end
...
AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
	...
	exports['t-notify']:Custom({
		...
		duration  =  10500,
		title  =  'EMS',
		message  =  'Du hast den Notruf angenommen, schaue schnell auf dein Navi und folge dem GPS.',
		sound  =  true
	})
          ...
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
               ...
               if IsControlJustReleased(0, 38) then
				...
				exports['t-notify']:Alert({
					style  =  'success',
					message  =  '✔️ Wiederbelebungsmaßnahmen sind im gange.'
				})
				...
				TriggerServerEvent('GMD-Ambulancejob:revive', XAid)
